...
Application::Application()
{
	...
	const char* vertex_shader_src = R"(
		...
		{	
			...
			vec3 L = normalize(u_lightPos.xyz - pos.xyz);
			vec3 E = normalize(-pos.xyz);
			vec3 N = normalize((u_modelView * vec4(a_normal, 0.0)).xyz);

			vec3 H = normalize(L + E);

			float LN = dot(L, N);
			float visible = step(0.0, LN);

			float Kd = LN * visible;
			vec3 diffuse = Kd * u_diffuseProduct;
			float Ks = pow(LN * visible, u_shininess);
			vec3 specular = Ks * u_specularProduct;

			v_color = pow(u_ambientProduct + diffuse + specular, vec3(1.0/u_gamma));
		}
	)";
